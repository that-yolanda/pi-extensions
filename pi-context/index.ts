...
function getReserveTokens(cwd: string): number {
	const DEFAULT = 16384;
	for (const p of [join(cwd, ".pi", "settings.json"), join(homedir(), ".pi", "agent", "settings.json")]) {
		if (!existsSync(p)) continue;
		try {
			const reserve = JSON.parse(readFileSync(p, "utf-8"))?.compaction?.reserveTokens;
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("context", {
    ...
    handler: async (args, ctx) => {
      ...
      const hasFullData = totalActual != null && limit > 0 && percent != null;
      const reserveTokens = hasFullData && totalActual > 0 ? getReserveTokens(ctx.cwd) : 0;
      ...
      const ratio = hasFullData && totalRaw > 0 ? (totalActual! / totalRaw) : 1;
      ...
      await ctx.ui.custom((tui, theme, kb, done) => {
        ...
        if (!hasFullData) {
          container.addChild(new Text(theme.fg("dim", " Context data not available yet"), 1, 0));
          container.addChild(new Text(theme.fg("dim", " Start a conversation to see usage"), 1, 0));
        } else {
          ...
          if (otherTokens > 10) categories.push({ key: "other", value: otherTokens });

          categories.push({ key: "available", value: Math.max(0, limit - reserveTokens - totalActual!) });
          ...
          while (blocks.length < totalBlocks) {
            blocks.push({ color: CATEGORY_CONFIG.available.color, symbol: `${CATEGORY_CONFIG.available.symbol} ` });
          }
          ...
          const catDetailLines = categories.map(cat => {
            ...
            return `${theme.fg(cfg.color as any, cfg.symbol)} ${theme.fg("text", labelStr)} ${theme.fg("accent", valStr)} (${rowPercent}%)`;
          });
