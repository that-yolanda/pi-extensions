...
const initSession = async (handles: HonchoHandles): Promise<Session> => {
	...
	await session.addPeers([
		[
			handles.userPeer,
			{
				observeMe: handles.config.observeMe,
				observeOthers: handles.config.observeOthers,
			},
		],
		[
			handles.aiPeer,
			{
				observeMe: handles.config.aiObserveMe,
				observeOthers: handles.config.aiObserveOthers,
			},
		],
	]);
...
export const ensureSession = async (
	...
	sessionPromise = initSession(handles).finally(() => {
		sessionPromise = null;
	});
...
export const bootstrap = async (
	...
	if (config.linkedHosts.length > 0) {
		...
		const settled = await Promise.allSettled(
			config.linkedHosts.map(
				async (hostName): Promise<LinkedHostHandle | null> => {
		...
		for (const result of settled) {
			if (result.status === "fulfilled" && result.value)
				...
				console.error(
					"[honcho-memory] linked host bootstrap failed:",
					result.reason,
				);
			}
		}
