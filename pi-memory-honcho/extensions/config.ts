...
export const normalizeRecallMode = (value: string | undefined): RecallMode => {
	if (value === "hybrid" || value === "context" || value === "tools")
		return value;
	if (value === "auto") return "hybrid";
	return "hybrid";
};

export const normalizeReasoningLevel = (
	value: string | undefined,
): ReasoningLevel => {
	if (
		value === "minimal" ||
...
const normalizeWriteFrequency = (
	...
	if (typeof value === "string") {
		...
		if (value === "async" || value === "turn" || value === "session")
			return value;
	}
	return "async";
};

const normalizeInjectionFrequency = (
	value: string | undefined,
): InjectionFrequency => {
...
const normalizeObservationMode = (
	value: string | undefined,
): ObservationMode => {
...
export const setRecallMode = (mode: RecallMode): void => {
	activeRecallMode = mode;
};
...
export const resolveConfig = async (): Promise<HonchoConfig> => {
	...
	const sessionStrategy = normalizeSessionStrategy(
		process.env.HONCHO_SESSION_STRATEGY ?? host.sessionStrategy,
	);
	const recallMode = normalizeRecallMode(
		process.env.HONCHO_RECALL_MODE ?? host.recallMode,
	);
	const contextTokens = toPositiveInt(
		process.env.HONCHO_CONTEXT_TOKENS ?? host.contextTokens,
		DEFAULT_CONTEXT_TOKENS,
	);
	const contextRefreshTtlSeconds = toPositiveInt(
		process.env.HONCHO_CONTEXT_REFRESH_TTL_SECONDS ??
			host.contextRefreshTtlSeconds,
	...
	const searchLimit = toPositiveInt(
		process.env.HONCHO_SEARCH_LIMIT ?? host.searchLimit,
		DEFAULT_SEARCH_LIMIT,
	);
	const toolPreviewLength = toPositiveInt(
		process.env.HONCHO_TOOL_PREVIEW_LENGTH ?? host.toolPreviewLength,
		DEFAULT_TOOL_PREVIEW_LENGTH,
	);
	const reasoningLevel = normalizeReasoningLevel(
		process.env.HONCHO_REASONING_LEVEL ?? host.reasoningLevel,
	);
	const contextInjectionInterval = toPositiveInt(
		process.env.HONCHO_CONTEXT_INJECTION_INTERVAL ??
			host.contextInjectionInterval,
		1,
	);

	// Observation: preset first, then explicit overrides
	const obsMode = normalizeObservationMode(
		process.env.HONCHO_OBSERVATION_MODE ?? host.observationMode,
	);
	const explicitObs = {
		observeMe:
			host.observeMe !== undefined
				? toBool(process.env.HONCHO_OBSERVE_ME ?? host.observeMe, true)
				: undefined,
		observeOthers:
			host.observeOthers !== undefined
				? toBool(process.env.HONCHO_OBSERVE_OTHERS ?? host.observeOthers, true)
				: undefined,
		aiObserveMe:
			host.aiObserveMe !== undefined
				? toBool(process.env.HONCHO_AI_OBSERVE_ME ?? host.aiObserveMe, true)
				: undefined,
		aiObserveOthers:
			host.aiObserveOthers !== undefined
				? toBool(
						process.env.HONCHO_AI_OBSERVE_OTHERS ?? host.aiObserveOthers,
						true,
					)
				: undefined,
	};
	...
	// Phase 1 additions
	const saveMessages = toBool(
		process.env.HONCHO_SAVE_MESSAGES ?? host.saveMessages,
		true,
	);
	const writeFrequency = normalizeWriteFrequency(
		process.env.HONCHO_WRITE_FREQUENCY ?? host.writeFrequency,
	);
	const dialecticDynamic = toBool(
		process.env.HONCHO_DIALECTIC_DYNAMIC ?? host.dialecticDynamic,
		true,
	);
	const dialecticMaxChars = toPositiveInt(
		process.env.HONCHO_DIALECTIC_MAX_CHARS ?? host.dialecticMaxChars,
		DEFAULT_DIALECTIC_MAX_CHARS,
	);
	const dialecticMaxInputChars = toPositiveInt(
		process.env.HONCHO_DIALECTIC_MAX_INPUT_CHARS ?? host.dialecticMaxInputChars,
		DEFAULT_DIALECTIC_MAX_INPUT_CHARS,
	);
	const sessionPeerPrefix = toBool(
		process.env.HONCHO_SESSION_PEER_PREFIX ?? host.sessionPeerPrefix,
		false,
	);
	const injectionFrequency = normalizeInjectionFrequency(
		process.env.HONCHO_INJECTION_FREQUENCY ?? host.injectionFrequency,
	);
	const contextCadence = toPositiveInt(
		process.env.HONCHO_CONTEXT_CADENCE ?? host.contextCadence,
		1,
	);
	const dialecticCadence = toPositiveInt(
		process.env.HONCHO_DIALECTIC_CADENCE ?? host.dialecticCadence,
		1,
	);
	const reasoningLevelCapRaw =
		process.env.HONCHO_REASONING_LEVEL_CAP ?? host.reasoningLevelCap;
	const reasoningLevelCap = reasoningLevelCapRaw
		? normalizeReasoningLevel(reasoningLevelCapRaw)
		: null;
	const envRaw =
		process.env.HONCHO_ENVIRONMENT ?? host.environment ?? "production";
	const environment: "local" | "production" =
		envRaw === "local" ? "local" : "production";
	const logging = toBool(process.env.HONCHO_LOGGING ?? file?.logging, true);
	const sessions = file?.sessions ?? {};
	const contextRefreshMessageThreshold =
		file?.contextRefresh?.messageThreshold ?? null;

	const enabled = process.env.HONCHO_ENABLED
