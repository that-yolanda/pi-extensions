...
const reconstructChunkedText = (chunks: string[]): string =>
	chunks
		.map((chunk, index) =>
			index === 0 ? chunk : chunk.replace(/^\[continued\] /, ""),
		)
		.join("");
...
const makeHandles = (sent: Array<unknown[]>): HonchoHandles => ({
	...
	userPeer: {
		message: (text: string) => ({ role: "user", text }),
	} as unknown as HonchoHandles["userPeer"],
	aiPeer: {
		message: (text: string) => ({ role: "assistant", text }),
	} as unknown as HonchoHandles["aiPeer"],
	session: {
		addMessages: async (msgs: unknown[]) => {
			sent.push(msgs);
		},
...
const fakeMessages = (
	texts: Array<{ role: "user" | "assistant"; content: string }>,
) =>
	texts.map((t) => ({
		role: t.role,
		content: t.content,
...
describe("WriteScheduler", () => {
	...
	test("session mode accumulates until flush", async () => {
		...
		await scheduler.onTurnEnd(
			handles,
			fakeMessages([{ role: "user", content: "turn 1" }]),
		);
		await scheduler.onTurnEnd(
			handles,
			fakeMessages([{ role: "user", content: "turn 2" }]),
		);
	...
	test("N-turn mode flushes every N turns", async () => {
		...
		await scheduler.onTurnEnd(
			handles,
			fakeMessages([{ role: "user", content: "t1" }]),
		);
		await scheduler.onTurnEnd(
			handles,
			fakeMessages([{ role: "user", content: "t2" }]),
		);
		expect(sent.length).toBe(0);

		await scheduler.onTurnEnd(
			handles,
			fakeMessages([{ role: "user", content: "t3" }]),
		);
	...
	test("async mode enqueues without blocking onTurnEnd", async () => {
		...
		await scheduler.onTurnEnd(
			handles,
			fakeMessages([{ role: "user", content: "async msg" }]),
		);
	...
	test("reset clears pending and turn count", async () => {
		...
		await scheduler.onTurnEnd(
			handles,
			fakeMessages([{ role: "user", content: "msg" }]),
		);
		scheduler.reset();
		await scheduler.flush();
	...
	test("chunks long messages according to maxMessageLength", async () => {
		...
		await scheduler.onTurnEnd(
			...
			fakeMessages([
				{
					role: "user",
					content: "This is a longer message that exceeds the limit",
				},
			]),
		);
...
describe("sanitizeCredentials", () => {
	...
	test("redacts npm tokens (standalone)", () => {
		const input =
			"token is npm_aBcDeFgHiJkLmNoPqRsTuVwXyZ0123456789ab right here";
		const result = sanitizeCredentials(input);
	...
	test("redacts npm tokens (with keyword prefix)", () => {
		...
		const input =
			"//registry.npmjs.org/:_authToken=npm_aBcDeFgHiJkLmNoPqRsTuVwXyZ0123456789ab";
		const result = sanitizeCredentials(input);
		expect(result).not.toContain("npm_aBcDeFgHiJkLmNoPqRsTuVwXyZ0123456789ab");
		expect(result).toContain("<REDACTED>");
	});
