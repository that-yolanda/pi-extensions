...
const repoBase = async (cwd: string): Promise<string | null> => {
	...
	if (root?.code === 0 && root.stdout.trim()) {
		...
		return sanitize(
			`${normalized.split("/").pop() ?? "repo"}_${hash(normalized)}`,
		);
	}
	return null;
};
...
export const deriveSessionKey = async (
	...
	if (manual) {
		...
		return config.sessionPeerPrefix
			? sanitize(`${config.peerName}_${key}`)
			: key;
	}
	...
	else if (strategy === "pi-session")
		key = sanitize(`pi_${hash(cwd)}_${Date.now().toString(36)}`);
	else if (strategy === "per-directory") key = directoryKey(cwd);
	else if (strategy === "per-repo")
		key = (await repoBase(cwd)) ?? directoryKey(cwd);
	else {
