...
const statusText = async (): Promise<string> => {
	...
	const sessionSource =
		handles?.sessionKey && config.sessions[process.cwd()]
			? "manual"
			: "derived";
	return [
...
export const registerCommands = (pi: ExtensionAPI): void => {
	...
	pi.registerCommand("honcho:setup", {
		...
		handler: async (_args, ctx) => {
			...
			const workspace = await ctx.ui.input(
				"Honcho workspace",
				existing.workspace,
			);
		...
		handler: async (_args, ctx) => {
			...
			const safe = {
				...
				apiKey: config.apiKey
					? `${config.apiKey.slice(0, 6)}...redacted`
					: undefined,
			};
	...
	pi.registerCommand("honcho:interview", {
		description:
			"Kick off a short preference interview and save a durable summary",
		handler: async (_args, ctx) => {
			const handles = getHandles();
			if (!handles) {
			...
			const preference = await ctx.ui.input(
				"What should PI remember about how you like to work?",
			);
			if (!preference) {
	...
	pi.registerCommand("honcho:doctor", {
		description:
			"Run a quick Honcho preflight for config, connectivity, and session mapping",
		handler: async (_args, ctx) => {
			const config = await resolveConfig();
			const checks: string[] = [];
			...
			checks.push(
				`linked_hosts: ${config.linkedHosts.length > 0 ? config.linkedHosts.join(", ") : "none"}`,
			);

			if (!config.enabled || !config.apiKey) {
				ctx.ui.notify(
					[...checks, "connectivity: skipped (Honcho not configured)"].join(
						"\n",
					),
					"warning",
				);
	...
	pi.registerCommand("honcho:map", {
		...
		handler: async (args, ctx) => {
			...
			ctx.ui.notify(
				`Session mapping: ${ctx.cwd} → ${name}\nNote: add to ~/.honcho/config.json sessions manually for persistence.`,
				"info",
			);
		},
	});
