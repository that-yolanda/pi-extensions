...
export const refreshCachedContext = async (
	...
	try {
		...
		console.error(
			"[honcho-memory] context refresh failed:",
...
export const shouldRefreshCachedContext = (handles: HonchoHandles): boolean => {
	...
	const ttlExpired =
		(Date.now() - cachedContext.refreshedAt) / 1000 >=
		handles.config.contextRefreshTtlSeconds;
	const thresholdExceeded =
		handles.config.contextRefreshMessageThreshold !== null &&
...
export const renderCachedContext = (contextTokens: number): string | null => {
	const sections = [
		...
		cachedContext.peerCard?.length
			? `User peer card:\n${cachedContext.peerCard.join("\n")}`
			: null,
		cachedContext.aiProfile
			? `AI peer profile:\n${cachedContext.aiProfile}`
			: null,
		cachedContext.aiCard?.length
			? `AI peer card:\n${cachedContext.aiCard.join("\n")}`
			: null,
		cachedContext.summary ? `Project summary:\n${cachedContext.summary}` : null,
		cachedContext.refreshedAt
			? `Context refreshed: ${new Date(cachedContext.refreshedAt).toISOString()}`
			: null,
